(function(){
	...
	function initFloatingAnimation(){
		var scroll_top = $(document).scrollTop();
		if(scroll_top >= 100){
			$('.floating').removeClass('floating-hidden');
			$('.floating').addClass('floating-visible');
		}else{
			$('.floating').removeClass('floating-visible');
			$('.floating').addClass('floating-hidden');
		}
	}
	...
	function initContactForm(){
		$("#c-form").validator().on("submit", function (e){
			if (e.isDefaultPrevented()){
				submitMSG(false, "No debes dejar nada vacío!");
			} else{
				e.preventDefault();
				var name = $("#name").val();
				var email = $("#email").val();
				...
				$.ajax({
					...
					success : function(text){
						if (text == "success"){
							...
							submitMSG(true, "Gracias! estaremos en contacto pronto :)")
